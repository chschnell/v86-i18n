...
def parse_codepages(cp_files, cp437_filepath, ibmgraph_filepath):
...
def main():
    parser = argparse.ArgumentParser(prog='import_cp', description='Generates 8-bit codepage to Unicode mappings.')
    ...
    codepages = parse_codepages(cp_files, cp437_filepath, ibmgraph_filepath)
    ...
    with open(args.out_filename, 'w', encoding='utf-8') as f_out:
        ...
        for i_cp_id, cp_id in enumerate(sorted_cp_ids):
